...
const Contact = () => {
  ...
  return (
    <div
      name="Contacts"
      className="relative max-w-screen-2xl container mx-auto px-4 md:px-20 mt-10 my-16"
    >
      <h1 className="text-3xl mb-3 font-bold">Contact me</h1>
          ...
          <div className="flex flex-col mb-4">
            ...
            <input
              {...register("email", {
                ...
                pattern: {
                  value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                  message: "Invalid email address. Must include '@' and '.com'."
                }
              })}
              className="border rounded-lg py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-none"
              type="text"
      ...
      {isLoading && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-85 z-50">
          <div className="text-white text-xl">Please wait, sending your message...</div>
        </div>
      )}
    </div>
  );
